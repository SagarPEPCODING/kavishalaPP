...
export default class EventPage extends Component {
  ...
  constructor(props) {
    ...
    this.registerChapter = () => {
      ...
      if (Auth.isAuthenticated()) {
        Api.registerChapter(this.state.chapter.slug, method).then(
          (response) => {
  ...
  static getInitialProps(ctx) {
    return Api.getChapterInfo(ctx.query.chapterslug).then((response) => {
      if (response.status === 200) {
        return {
  ...
  componentDidMount() {
    Api.checkRegistration("chapter", this.props.chapter.slug).then(
      (response) => {
        if (response.status === 200) {
          this.setState({ isRegistered: response.data.registered });
        }
      }
    );
  }
